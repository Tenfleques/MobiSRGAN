...
def lr_images(images_real , downscale):
    ...
    for img in  range(len(images_real)):
        width = int(images_real[img].shape[0]//downscale)
        height = int(images_real[img].shape[1]//downscale)
        dim = (width, height)

        lr = cv2.resize(images_real[img], dim, interpolation=cv2.INTER_CUBIC)
...
def load_data_from_dirs(dirs, ext, patch_size=(96, 96)):
    files = []
    file_names = []
    count = 0
    for d in dirs:
