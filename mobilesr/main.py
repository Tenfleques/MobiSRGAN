...
@tf.function
def train_step(model, x, y, lr_model=None):
    ...
    with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
        ...
        if lr_model is not None:
            fake_lr = lr_model.predict(fake_hr)
            cyclic_loss = tf.keras.losses.MeanSquaredError()(x, fake_lr)

            mse_loss += cyclic_loss
            content_loss += model.content_loss(x, fake_lr)
...
def train(model, dataset, log_iter, writer, lr_model=None):
    """
    ...
    with writer.as_default():
        ...
        for x, y in dataset:
            disc_loss, adv_loss, content_loss, mse_loss = train_step(model, x, y, lr_model=lr_model)
...
def main():
    ...
    if args.lr_model_weights:
        ...
        output = lr_model.model(inputs)
        lr_model = tf.keras.models.Model(inputs, output)
        
    # Create the tensorflow dataset.
    ds = DataLoader(args.image_dir, args.hr_size, lr_model).dataset(args.batch_size)
    ...
    for _ in range(args.epochs):
        train(gan, ds, args.save_iter, train_summary_writer, lr_model)
